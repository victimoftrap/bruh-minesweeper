...
export default class GameEngine {
    ...
    public static newGame(level: GameLevel): Game {
      const gameField: GameField = new GameField(level.rows, level.columns);
    ...
    public static isMinesCovered(): boolean {
      return this.game.field.fieldArray
        .filter((cell: Cell) => cell.isMine)
        .filter((minedCell: Cell) => minedCell.state !== CellState.FLAGGED)
        .length === 0;
    }
}
